...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        message.channel.send(`:check: **Are you sure you want to ban ${bannedCollection.map(mem => `${mem.toString()}`).join(", ")}?** \nPlease reply with \`y\`/\`yes\` or \`cancel\``)
        collector.on("collect", async msg => {
            if (['yes', 'y'].includes(msg.content.toLowerCase())) {
                ...
                collector.stop()
                return message.channel.send("Massban", "I have cancelled the massban on " + bannedCollection.size + " members", client.colors.green)
            } else {
