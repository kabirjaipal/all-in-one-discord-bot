        ...
        if (!user) {
            return message.channel.send(
                new MessageEmbed()
                    .setColor("RED")
                    .setAuthor(message.author.tag)
                    .setDescription("**Please Mention the person to who you want to warn - warn @mention <reaosn>**")
                    .setFooter("Coded by: Tech Boy Gaming")
            ).then((msg => {
                msg.delete({ timeout: 10000 })
            }))
        ...
        if (!reason) {
            return message.channel.send(
                new MessageEmbed()
                    .setColor("RED")
                    .setAuthor(message.author.tag)
                    .setDescription("**Please provide reason to warn - warn @mention <reason>**")
                    .setFooter("Coded by: Tech Boy Gaming")
            ).then((msg => {
                msg.delete({ timeout: 10000 })
            }))
        ...
        if (warnings === null) {
            ...
            await message.channel.send(
                new MessageEmbed()
                    ...
                    .setAuthor(message.author.tag)
                    .setThumbnail(user.user.displayAvatarURL())
                    .setDescription(`✅ Success |  You warned **${user.user.username}** for ${reason}`)
