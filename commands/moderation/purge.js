    ...
    run: async (client, message, args) => {
        ...
        try {
            ...
            if (!message.member.permissions.has("MANAGE_MESSAGES")) return  message.channel.send(
                ...
                .setDescription(` 🥺 You don't have **MANAGE_MESSAGES** premssions to use this command.`)
            ...
            let error1 = new MessageEmbed()
                ...
                .setAuthor(message.author.tag)
                .setThumbnail(message.author.displayAvatarURL())
                .setDescription(` 😂😂 You can only delete the messages which are not older than 14 days.`)
                .setFooter("Coded by: Tech Boy Gaming")
                .setTimestamp()
            ...
            if (isNaN(amount) || !Number.isInteger(amount)) return functions.embedbuilder(client, 9000, message, config.colors.no, "😂😂 Please enter a number of messages to purge.")
            if (!amount || amount < 2 || amount > 100) return functions.embedbuilder(client, 9000, message, config.colors.no, "😂😂 Please enter a number of message between 2 and 100.")
            if (!args[1]) {
