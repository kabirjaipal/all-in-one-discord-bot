        ...
        if (args[0].toLowerCase() === "create") {
            ...
            if (args[1]) {
                try {
                    ...
                    backup.list(guild).then((backups) => {
                        ...
                        embed.addField(`GET INFORMATION", "to get info about a Backup, type: ${prefix}backup info <ID>`)
                        message.channel.send(embed)
                    });
                } catch {
                    return message.reply("CANNOT GET INFORMATION ABOUT THIS SERVER ID!")
            ...
            if (args[1]) {
                try {
                    ...
                    let backupID = args[2];
                    if (!backupID) {
                        return message.channel.send(`:x: | You must specify a valid backup ID!: ${prefix}backup loadother <SERVERID> <ID>`);
                    }
                    message.reply("LOADING...")
