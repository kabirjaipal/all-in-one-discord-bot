    ...
    scdl
      ...
      .then(async (results) => {
        ...
        for (let i = 0; i < results.collection.length; i++) {
          try {
            let mins = Math.floor(
              results.collection[i].full_duration / 1000 / 60
            );
            let secs = Math.floor(
              (results.collection[i].full_duration / 1000) % 60
            );
            ...
            searchresult += await `**${i + 1}**. [${
              results.collection[i].permalink
            }](${results.collection[i].permalink_url}) - \`${durr}\`\n`;
          } catch {
            searchresult += await " ";
          }
        }
        ...
        functions.embedbuilder(
          ...
          config.colors.yes,
          "🔎 Searching!",
          `[${results.collection[userinput - 1].permalink}](${
            results.collection[userinput - 1].permalink_url
          })`,
          results.collection[userinput - 1].artwork_url
        );
